...
public class UserUI {
    ...
    public int menu() {
        ...
        System.out.println("3. 회원 정보수정하기");
        System.out.println("4. 회원 정보삭제하기");
        System.out.println("5. 종료");
        int menuId = -1;
        try {
            String line = br.readLine();
            menuId=Integer.parseInt(line);//문자열을 정수로 바꿔준다.
        } catch (Exception ex) {
            ex.printStackTrace();
        }
        return menuId;
...
//유저정보등록
    ...
    //유저정보 출력
    public void printUsersList(Iterator<User> iter){
        System.out.println("emil        name         생년");
        System.out.println("=============================");
        while(iter.hasNext()){
            User user=iter.next();
